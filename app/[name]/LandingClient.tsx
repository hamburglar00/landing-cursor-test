...
function normEmail(v: string | null) { return (v || "").trim().toLowerCase(); }
...
export default function LandingClient({ config }: { config: LandingPublicConfig }) {
  ...
  useEffect(() => {
    if (!pickedRef.current) {
      pickedRef.current = (async () => {
        const base = process.env.NEXT_PUBLIC_SUPABASE_URL; // opcional
        // Mejor: usar el mismo SUPABASE_EDGE_BASE pero en client no está.
        // Solución: usar un endpoint público en tu landing que proxee, pero por ahora:
        // Vamos directo a Supabase Edge con env público:
        const edgeBase = (process.env.NEXT_PUBLIC_SUPABASE_URL || "").replace(".supabase.co","") 
          ? null : null;

        // Para evitar lío de env: lo dejamos a URL absoluta por variable pública:
        const EDGE = (window as any).__EDGE_BASE || ""; // si querés setearlo inline después
        // Si no lo seteás, ponelo directo acá hardcodeado en tu deploy.
        const url = `${(window as any).__SUPABASE_EDGE_BASE || ""}/functions/v1/landing-phone?name=${encodeURIComponent(config.name)}`;
        ...
        return { number, name: data?.name, meta: data };
      })();
    }
  }, [config.name]);
  ...
  async function onClick(e: React.MouseEvent){
    ...
    try{
      ...
      // fallback: si no hay phone, no hacemos nada
      setOverlay(false);
      return;
    }
    ...
    try{
      ...
      if (window.fbq) fbq("track","Contact",
        { content_name:"Botón WhatsApp", content_category:"LeadGen", event_source:"LandingPage", source:"main_button" },
        { eventID: event_id }
      );
    }catch{}
